...
func NewInputField(canvas fyne.Canvas, handler input2.Handler) *InputField {
...
func newInputField(canvas fyne.Canvas, handler input2.Handler) *InputField {
	...
	newInput.inputHandler.BindFunctionToAction(newInput, input2.ConfirmAction, func() { newInput.OnConfirm() })
	newInput.inputHandler.BindFunctionToAction(newInput, input2.ExitInputModeAction, func() { newInput.ExitInputMode() })
	return newInput
}
...
func (inputField *InputField) TypedKey(key *fyne.KeyEvent) {
	...
	*/
	...
	if handled && handlingResult.Action == input2.ExitInputModeAction && handlingResult.KeyCombination.BothKeysPressed() {

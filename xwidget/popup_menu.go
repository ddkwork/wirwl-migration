...
func NewPopUpMenu(canvas fyne.Canvas, handler input2.Handler, choicesNames ...string) *PopUpMenu {
	...
	menu.inputHandler.BindFunctionToAction(menu, input2.MoveDownAction, func() { menu.selectNextChoice() })
	menu.inputHandler.BindFunctionToAction(menu, input2.MoveUpAction, func() { menu.selectPreviousChoice() })
	menu.inputHandler.BindFunctionToAction(menu, input2.ConfirmAction, func() { menu.onChoiceSelected() })
	menu.currentChoice().TextStyle = fyne.TextStyle{Bold: true}
	return menu
}

func generateChoicesFromNames(choicesNames ...string) []*fyneWidget.Label {
	var choices []*fyneWidget.Label

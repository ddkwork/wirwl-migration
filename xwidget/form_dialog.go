...
func NewFormDialogFormItemFactory(canvas fyne.Canvas, inputHandler input2.Handler) *FormDialogFormItemFactory {
...
func NewFormDialog(canvas fyne.Canvas, inputHandler input2.Handler, title string, formItems ...*FormDialogFormItem) *FormDialog {
...
func (dialog *FormDialog) setupInputHandler() {
	dialog.inputHandler.BindFunctionToAction(dialog, input2.MoveDownAction, func() {
		dialog.setCurrentInputTo(dialog.currentInputNum + 1)
	})
	dialog.inputHandler.BindFunctionToAction(dialog, input2.MoveUpAction, func() {
		dialog.setCurrentInputTo(dialog.currentInputNum - 1)
	})
	dialog.inputHandler.BindFunctionToAction(dialog, input2.EnterInputModeAction, func() {
		dialog.currentWidget().EnterInputMode()
	})
	dialog.inputHandler.BindFunctionToAction(dialog, input2.ConfirmAction, func() {
		dialog.handleEnterKey()
	})
	dialog.inputHandler.BindFunctionToAction(dialog, input2.CancelAction, func() {
